...
def _normalise_df(raw: object) -> Optional[pd.DataFrame]:
    ...
    if isinstance(raw, pd.DataFrame) and not raw.empty:
        if "player_name" in raw.columns:
            ...
            position_col = next((c for c in raw.columns if c.lower() in {"player_position_id", "position"}), None)
            if position_col:
                raw["player_name"] = raw.apply(
                    lambda row: clean_dst_name(row["player_name"]) if str(row[position_col]).upper() == "DST" else clean_player_name(row["player_name"]),
                    axis=1,
                )
            else:
