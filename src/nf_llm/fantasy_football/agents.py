...
class FantasyAgentSystem:
    ...
    def _clean_constraints(self, constraints: Dict) -> Dict:
        ...
        if "must_include" in constraints:
            cleaned["must_include"] = [
                name.replace(" ", "").lower() for name in constraints["must_include"]
            ]

        # Clean team abbreviations
        if "avoid_teams" in constraints:
            cleaned["avoid_teams"] = [
                team.lower() for team in constraints["avoid_teams"]
            ]

        if "stack_teams" in constraints:
            cleaned["stack_teams"] = [
                team.lower() for team in constraints["stack_teams"]
            ]
    ...
    def validate_lineup(self, lineup: Dict, user_input: str) -> bool:
        ...
        if "must_include" in constraints:
            ...
            if not all(
                player in lineup_players for player in constraints["must_include"]
            ):
                return False
    ...
    def explain_lineup(self, lineup: Dict, analysis_results: Dict) -> str:
        ...
        try:
            explanation_prompt = self._build_explanation_prompt(
                lineup, analysis_results
            )

            self.strategy_agent.reset()
            self.user_proxy.send(
                explanation_prompt, self.strategy_agent, request_reply=True
            )
