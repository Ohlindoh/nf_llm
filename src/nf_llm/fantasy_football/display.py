            ...
            if st.button("Show Value Analysis"):
                ...
                for position, plays in analysis["value_plays"].items():
                    if not plays.empty:
                        ...
                        formatted_plays["salary"] = formatted_plays["salary"].apply(
                            lambda x: f"${x:,.0f}"
                        )
                        formatted_plays["points_per_dollar"] = formatted_plays[
                            "points_per_dollar"
                        ].apply(lambda x: f"{x:.2f}")
                        formatted_plays["points_zscore"] = formatted_plays[
                            "points_zscore"
                        ].apply(lambda x: f"{x:.2f}")
                ...
                if analysis["stacks"]:
                    ...
                    formatted_stacks["total_salary"] = formatted_stacks[
                        "total_salary"
                    ].apply(lambda x: f"${x:,.0f}")
                    formatted_stacks["stack_value"] = formatted_stacks[
                        "stack_value"
                    ].apply(lambda x: f"{x:.2f}")
    ...
    def _display_lineup_analysis(self, lineup: Dict, analysis_results: Dict):
        ...
        with st.expander("👓 View Lineup Analysis", expanded=False):
            ...
            if stacks:
                ...
                for team in stacks:
                    stack_players = [
                        p["player_name"] for p in lineup.values() if p["team"] == team
                    ]
