...
class LineupOptimizer:
    ...
    def _validate_data(self):
        ...
        required_columns = {'player_name', 'player_position_id', 'team', 'salary', 'projected_points'}
    ...
    def generate_lineups(self, constraints: Dict) -> List[Dict]:
        ...
        for i in range(num_lineups):
            ...
            exposure_constraints = self._calculate_exposure_constraints(i + 1, max_exposure)
    ...
    def generate_lineup(self, constraints: Dict = None) -> Dict:
        ...
        for idx, scenario in enumerate(scenarios):
            ...
            if lineup:
                ...
                print(f"Scenario {idx + 1} produced valid lineup with score: {score}")  # Debug print
                lineups.append(lineup)
                scores.append(score)
        
        print(f"Found {len(lineups)} valid lineups out of {total_scenarios} scenarios")  # Debug print
            ...
            for position, plays in analysis_results['value_plays'].items():
                for play in plays:  # plays is now a list of dictionaries
                    ...
                    self.strategy_boost[play['player_name']] = max(
                        self.strategy_boost.get(play['player_name'], 0),
                        boost
                    )
        ...
        if 'stacks' in analysis_results:
            for stack in analysis_results['stacks'][:5]:  # Top 5 stacks
                ...
                self.strategy_boost[stack['qb']] = max(
                    self.strategy_boost.get(stack['qb'], 0),
                    qb_boost
                )
                self.strategy_boost[stack['receiver']] = max(
                    self.strategy_boost.get(stack['receiver'], 0),
                    receiver_boost
                )
    ...
    def _optimize_single_scenario(self, scenario: pd.DataFrame, constraints: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            base_vars = pulp.LpVariable.dicts("players",
            ...
            prob += pulp.lpSum(
                p['projected_points'] * (
                    ...
                    if p['player_position_id'] in self.settings.FLEX_POSITIONS else 0)
                )
                for p in players
            )
            ...
            if pulp.LpStatus[prob.status] == 'Optimal':
                print(f"Found optimal solution with status: {pulp.LpStatus[prob.status]}")  # Debug print
                lineup = self._build_lineup_from_solution(base_vars, flex_vars, players)
                if self._validate_lineup(lineup):
                    print(f"Built valid lineup with {len(lineup)} players")  # Debug print
                    return lineup
                else:
                    print("Built lineup failed validation")  # Debug print
                    return None
            else:
                print(f"No optimal solution found. Status: {pulp.LpStatus[prob.status]}")  # Debug print
                return None
    ...
    def _add_constraints(self, prob, base_vars, flex_vars, players, constraints):
        ...
        for pos, limit in self.settings.POSITIONS.items():
            if pos != 'FLEX':
                prob += pulp.lpSum(
                    base_vars[p['player_name'], p['player_position_id']]
                    for p in players if p['player_position_id'] == pos
                ) == limit

        # FLEX position constraint
        prob += pulp.lpSum(
            flex_vars[p['player_name'], p['player_position_id']]
            for p in players if p['player_position_id'] in self.settings.FLEX_POSITIONS
        ) == 1

        # Player can't be in both base and FLEX
        for p in players:
            if p['player_position_id'] in self.settings.FLEX_POSITIONS:
                prob += base_vars[p['player_name'], p['player_position_id']] + \
                        flex_vars[p['player_name'], p['player_position_id']] <= 1
    ...
    def _calculate_exposure_constraints(self, current_lineup_count: int, max_exposure: float) -> Dict:
    ...
    def _build_lineup_from_solution(self, base_vars, flex_vars, players: List[Dict]) -> Dict:
        ...
        for player in players:
            ...
            if pos in self.settings.FLEX_POSITIONS and is_selected(flex_vars[name, pos]):
                lineup['FLEX'] = player
                used_players.add(name)
                break
